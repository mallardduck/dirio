...
func RegisterRoutes(r *teapot.Router, h RouteHandlers) {
	...
	r.GET("/minio/health/live", h.HandleLive()).Name("minio.health.live").Action("minio:HealthLive")
	r.GET("/minio/health/ready", h.HandleReady()).Name("minio.health.ready").Action("minio:HealthReady")
}
